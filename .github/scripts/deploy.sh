...
BE_FETCH_SHA_TAG="${BE_FETCH_SHA_TAG:-}"
...
echo "BE_FETCH_SHA_TAG: ${BE_FETCH_SHA_TAG}"
...
if [[ "${IS_SWARM}" == "true" ]]; then
    ...
    if [ -z "${BE_FETCH_SHA_TAG}" ]; then
        BE_FETCH_SHA_TAG=$(docker service inspect "${project_container_name}_be-fetch" --pretty | grep image= | awk -F: '{print $2}')
        echo "BE Fetch did not change. Using $BE_FETCH_SHA_TAG tag."
    fi
...
sed -i "s/BE_FETCH_TAG=/BE_FETCH_TAG=$BE_FETCH_SHA_TAG/g" .env
...
if [[ "${IS_SWARM}" != "true" ]]; then
    ...
    if [ -z "${BE_FETCH_SHA_TAG}" ]; then
        # If BE Fetch did not change - remove it from the list of services for deployment.
        echo "BE Fetch will not be deployed."
        deploy_services=$(echo "${deploy_services}" | tr ' ' '\n' | grep -v '^api$' | tr '\n' ' ')
    fi
